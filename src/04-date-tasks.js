...
function timeSpanToString(startDate, endDate) {
  ...
  let remainder = difference - hour * 3600000;
  const minutes = Math.floor((remainder / 60000));
  remainder -= minutes * 60000;
  const seconds = Math.floor((remainder / 1000));
  remainder -= seconds * 1000;
  const milliseconds = remainder;
  const hourTemp = ((hour < 10) ? '0' : '') + hour;
  const minutesTemp = ((minutes < 10) ? '0' : '') + minutes;
  const secondsTemp = ((seconds < 10) ? '0' : '') + seconds;
  const millisecondsTemp = ((milliseconds < 10) ? '00' : '') + milliseconds;
  return `${hourTemp}:${minutesTemp}:${secondsTemp}.${millisecondsTemp}`;
}
